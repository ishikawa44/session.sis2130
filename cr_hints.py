...
cursor.execute('''CREATE TABLE IF NOT EXISTS hints
                (record_number INTEGER PRIMARY KEY, generated_password TEXT)''')
conn.commit()
conn.close()
...
if not record:
    cursor.execute("INSERT INTO hints (record_number, generated_password) VALUES (1, 'Пароль не задан')") #Вставляет новую запись в таблицу "hints" с номером 1 и значением "Пароль не задан".
